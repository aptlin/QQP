...
class Bet:
    def __init__(self,
        ...
        self.STAMP = 'bet_%d_%.2f'%(num_dense,
                                   rate_drop_dense)
...
# * Model Constructor
    def _model_constructor(self):
        ...
        timedist_layer = TimeDistributed(Dense(self.EMBEDDING_DIM, activation=self.RECTIFIER))
        lambda_layer = Lambda(lambda x: K.max(x, axis=1), output_shape=(self.EMBEDDING_DIM, ))
        ...
        merged = Dense(round(1.5*self.NUM_DENSE),
                       kernel_initializer='normal',
                       activation=self.RECTIFIER,
                       kernel_constraint=maxnorm(3))(merged)
        merged = Dropout(self.RATE_DROP_DENSE)(merged)
        merged = BatchNormalization()(merged)
        merged = Dense(self.NUM_DENSE,
                       kernel_initializer='normal',
                       activation=self.RECTIFIER,
                       kernel_constraint=maxnorm(3))(merged)
        merged = Dropout(self.RATE_DROP_DENSE)(merged)
        ...
        merged = Dropout(self.RATE_DROP_DENSE)(merged)
        merged = BatchNormalization()(merged)
        merged = Dense(round(0.25 * self.NUM_DENSE),
                       kernel_initializer='normal',
                       activation=self.RECTIFIER,
                       kernel_constraint=maxnorm(3))(merged)
        merged = Dropout(self.RATE_DROP_DENSE)(merged)
